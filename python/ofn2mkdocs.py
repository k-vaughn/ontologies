...
def main():
    ...
    for ofn_path in sorted(ofn_files):
        ...
        try:
            ...
            g, ns, prefix_map, classes, local_classes = process_ontology(ofn_path, errors, ontology_info[ofn_path])
            if g is None:
                continue
            ...
            for cls in sorted(local_classes, key=lambda u: get_label(g, u).lower()):
                ...
                try:
                    ...
                    # Generate Markdown
                    generate_markdown(g, cls, cls_name, global_patterns, global_all_classes, ns, ofn_path, errors, prefix_map)
                    processed_count += 1
